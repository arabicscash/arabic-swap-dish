...
import './interfaces/IArabicMaster.sol';
...
contract DishMaster is ERC721Holder, Ownable, Pausable {
    ...
    uint256 public constant accAbicPerShareMultiple = 1E12;
    uint256 public lastRewardBlock;
    // total has deposit to ArabicMaster stakingPower
    ...
    address public arabicToken;
    address public dishStakingPowerToken;
    DishNFT public dishNFT;
    IArabicMaster public arabicMaster;
    ...
    constructor(
        ...
        address _arabicToken,
        address _dishNFT,
        address _arabicMaster,
        address _feeAddr
    ) public {
        dishStakingPowerToken = _dishStakingPowerToken;
        arabicToken = _arabicToken;
        dishNFT = DishNFT(_dishNFT);
        arabicMaster = IArabicMaster(_arabicMaster);
        feeAddr = _feeAddr;
        emit FeeAddressTransferred(address(0), feeAddr);
    }

    function approveArabicMasterForSpendStakingPowerToken() public {
        IERC20(dishStakingPowerToken).approve(address(arabicMaster), 2**256 - 1);
    }
    ...
    function synthesis(uint256 _amount) public {
        ...
        IERC20(arabicToken).safeTransferFrom(address(msg.sender), feeAddr, feeAmount);
        IERC20(arabicToken).safeTransferFrom(address(msg.sender), address(this), amount);
    ...
    function decomposition(uint256 tokenId) public {
        ...
        IERC20(arabicToken).safeTransfer(address(msg.sender), dishInfo.abicAmount);
        DishStakingPowerToken(dishStakingPowerToken).burn(dishInfo.stakingPowerAmount);
        dishNFT.burn(tokenId);
        emit Decomposition(msg.sender, tokenId, dishInfo.abicAmount);
        delete dishInfoMap[tokenId];
    }

    // View function to see pending ABICs on frontend.
    function pendingAbic(address _user) external view returns (uint256) {
        UserInfo memory userInfo = _userInfoMap[_user];
        uint256 _accAbicPerShare = accAbicPerShare;
        if (totalStakingPower != 0) {
            uint256 totalPendingAbic = arabicMaster.pendingAbic(dishStakingPowerToken, address(this));
            _accAbicPerShare = _accAbicPerShare.add(
                totalPendingAbic.mul(accAbicPerShareMultiple).div(totalStakingPower)
            );
        }
        return userInfo.amount.mul(_accAbicPerShare).div(accAbicPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updateStaking() public {
        ...
        (, uint256 lastRewardDebt) = arabicMaster.poolUserInfoMap(dishStakingPowerToken, address(this));
        arabicMaster.deposit(dishStakingPowerToken, 0);
        (, uint256 newRewardDebt) = arabicMaster.poolUserInfoMap(dishStakingPowerToken, address(this));
        accAbicPerShare = accAbicPerShare.add(
            newRewardDebt.sub(lastRewardDebt).mul(accAbicPerShareMultiple).div(totalStakingPower)
        );
        lastRewardBlock = block.number;
    }
    ...
    function stake(uint256 tokenId) public whenNotPaused {
        ...
        if (userInfo.amount != 0) {
            uint256 pending = userInfo.amount.mul(accAbicPerShare).div(accAbicPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending != 0) {
        ...
        if (tokenId != 0) {
            ...
            arabicMaster.deposit(dishStakingPowerToken, dishInfo.stakingPowerAmount);
            totalStakingPower = totalStakingPower.add(dishInfo.stakingPowerAmount);
        }
        userInfo.rewardDebt = userInfo.amount.mul(accAbicPerShare).div(accAbicPerShareMultiple);
        if (tokenId != 0) {
    ...
    function unstake(uint256 tokenId) public {
        ...
        DishInfo memory dishInfo = dishInfoMap[tokenId];
        updateStaking();
        uint256 pending = userInfo.amount.mul(accAbicPerShare).div(accAbicPerShareMultiple).sub(userInfo.rewardDebt);
        if (pending != 0) {
            safeAbicTransfer(msg.sender, pending);
        }
        ...
        arabicMaster.withdraw(dishStakingPowerToken, dishInfo.stakingPowerAmount);
    ...
    function emergencyUnstakeAll() public onlyOwner whenPaused {
        arabicMaster.emergencyWithdraw(dishStakingPowerToken);
    ...
    function safeAbicTransfer(address _to, uint256 _amount) internal {
        uint256 abicBal = IERC20(arabicToken).balanceOf(address(this));
        if (_amount > abicBal) {
            IERC20(arabicToken).transfer(_to, abicBal);
        } else {
            IERC20(arabicToken).transfer(_to, _amount);
        }
    }
